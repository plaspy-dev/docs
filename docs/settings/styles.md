...
---
# Styles
The "Styles" section in Plaspy's [settings](https://app.plaspy.com/Settings) allows administrators to customize the platform's appearance by uploading CSS \(Cascading Style Sheets\). This functionality is useful for adapting the interface for both desktop and mobile devices, ensuring a consistent and professional user experience. This guide details the available fields and the steps to configure them properly.

![](./image_1718312415297.gif)

### Field Descriptions

- **Load Default CSS Styles**: Option to enable or disable the use of Plaspy's default CSS styles. Enabling this option will automatically adapt Plaspy's current theme to the primary color of the website without needing to include any additional CSS scheme.
...
- **Load Default CSS Styles**: If this option is enabled, Plaspy will apply its default CSS styles, adjusting the theme to the primary color configured on the platform.
...
- **Can I change the CSS stylesheet at any time?**
    - Yes, you can update the URLs of the CSS stylesheets at any time from the "Styles" section in Plaspy's settings. Just make sure to save the changes after making any modifications.
- **What are the benefits of enabling default CSS styles?**
    - Enabling default CSS styles will automatically adapt Plaspy's theme to the primary color configured on the platform, simplifying the customization process without the need for additional CSS schemes.

With these instructions, you can effectively configure the "Styles" section and customize the appearance of the Plaspy platform according to your organization's needs.
