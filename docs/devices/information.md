...
---
# Information
The [Device](https://app.plaspy.com/Devices) Information section provides key details about the status and characteristics of the selected device. This information is crucial for effectively monitoring and managing each device in your account. Here you will find data such as the device's creation date, last connection, protocol used, associated phone number, and more.

![](./image_1718310114700.gif)

## Field Descriptions

- **Creation**: This field shows the date and time when the device was added to your account. This information is useful for tracking how long the device has been in use.
- **Last Connection**: Indicates the date and time of the last time the device connected to the network. This helps you verify if the device is active and functioning correctly.
- **Protocol**: Displays the communication protocol used by the device. automatically detects the protocol, but if there are similar or compatible protocols for your device, a list of compatible protocols will appear. Usually, detects the tracker protocols, and it is not necessary to modify it in most cases.
...
- [***fa-print* Certificate**:](certificates) Generates an official certificate for the device, indicating relevant information that you can provide to third parties to show that the device is being tracked by.
- ***fa-bug* Debug Device**: This option allows you to check the tracker’s connection in real-time. The information is displayed raw and, if possible, in text format. If not possible, it will be shown in hexadecimal format, for example, 0x686F6C61 for binary information.
...
3. You will see the protocol that the device uses to communicate. If there are similar or compatible protocols, a list of options will appear. Usually, it is not necessary to modify the protocol detected by.
