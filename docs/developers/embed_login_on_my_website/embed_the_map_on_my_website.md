...
Embedding the map with the position of your devices on your website facilitates real-time visualization of device locations for visitors. This allows users to see the locations without needing direct access to, and you can protect this visualization using your own authentication mechanism, independent 's login credentials.
...
1. **Enable API access for your account:** Follow the instructions to [enable the API](../enable_the_api) in your account. This includes generating an API Key.
...
3. **Create a group with the devices you want to display:**, create a [group](https://app.plaspy.com/Groups) and include all the devices you want to visualize on the map.
...
- **Security:** Verify that your website is configured to securely handle communication with the API.
...
Embedding the map on your website is an effective way to provide visitors with access to real-time positions of your devices. By following the mentioned steps, you can easily integrate the map using the API, facilitating visualization and improving the user experience on your site.
